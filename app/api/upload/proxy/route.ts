...
const IMAGE_CATEGORIES = ["image", "event-image", "admin-profile-image", "member-profile-image"] as const;

async function getUploadOwner(category: string) {
    try {
        if (category === "admin-profile-image") return (await requireAdmin()).user.id;
...
export async function POST(request: NextRequest) {
    try {
        ...
        const parsed = proxySchema.safeParse({ category: categoryRaw });
        if (!parsed.success) {
        ...
        return NextResponse.json({ key, publicUrl: `${R2_PUBLIC_URL}/${key}` });
    } catch (err) {
