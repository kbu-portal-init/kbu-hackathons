    ...
    return (
        <main className="flex flex-1 items-center bg-background px-6 py-16 dot-grid">
            <section className="mx-auto w-full max-w-4xl">
                ...
                <div className="mt-10 grid gap-5 md:grid-cols-2">
                    {loginOptions.map(({ href, icon: Icon, label, description }) => (
                        <Link
                            key={href}
                            href={href}
                            className="group rounded-2xl glass-violet p-7 transition duration-300 hover:-translate-y-1 hover:border-violet-500/50"
                        >
                            <div className="flex size-12 items-center justify-center rounded-xl bg-violet-500/10 text-violet-400 transition group-hover:bg-violet-500/20">
                                <Icon className="size-6" />
                            </div>
                            <h2 className="mt-6 text-2xl font-bold text-foreground">{label} login</h2>
                            <p className="mt-3 text-sm leading-6 text-muted-foreground">{description}</p>
                            <span className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-cyan-500">
                                Continue{" "}
                ...
                <Link
                    href="/"
                    className="mx-auto mt-8 block w-fit text-sm font-medium text-muted-foreground transition hover:text-cyan-400"
                >
                    Back to KBU Hub
                </Link>
            </section>
